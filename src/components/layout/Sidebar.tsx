  ...
  const sidebarContent = (
    <>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
          ...
          <div className="mt-6 rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <Zap className={cn("h-3.5 w-3.5", creditsLow ? "text-red-500" : "text-teal-600 dark:text-teal-400")} />
                <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">
                  {isPro ? "AI Credits" : "Trial Credits"}
                </span>
              </div>
              <span className={cn("text-xs font-medium tabular-nums", creditsLow ? "text-red-600 dark:text-red-400" : "text-gray-600 dark:text-gray-400")}>
            ...
            <div className="h-1.5 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
              <div
                className={cn(
                  ...
                  creditsLow ? "bg-red-500" : creditsPercent <= 50 ? "bg-amber-500" : "bg-teal-600"
                )}
        ...
        {!isPro && (
          <div className="mt-6 rounded-lg bg-teal-50 dark:bg-teal-900/30 p-4">
            ...
            <Link
              href="/upgrade"
              onClick={() => setMobileOpen(false)}
              className="block w-full rounded-md bg-teal-700 px-3 py-1.5 text-center text-xs font-medium text-white hover:bg-teal-800 dark:hover:bg-teal-600"
            >
              Upgrade Now
            </Link>
          </div>
          ...
          {isAppAdmin && (
            <li>
              <Link
                href="/admin"
                onClick={() => setMobileOpen(false)}
                className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-red-600 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-700 dark:hover:text-red-300"
              >
    ...
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 flex h-14 items-center gap-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate max-w-[180px]">{orgName}</span>
        </div>
      </div>
